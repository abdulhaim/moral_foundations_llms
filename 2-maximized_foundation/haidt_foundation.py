...
def save_prompt_responses(engine, prompt, foundation, model_answers):
	...
	file_name = base_folder + "/" + engine + "/engine_" + engine + "_prompt_f" + foundation + "_" + prompt_refactor
...
def run_one_prompt(engine, prompt, foundation):
	model_answers = run_prompts(prompt, engine)
	save_prompt_responses(engine, prompt, foundation, model_answers)
	return
...
def for_one_engine(engine):
	...
	for foundation, sentence in maximized_prompt_dictionary.items():
